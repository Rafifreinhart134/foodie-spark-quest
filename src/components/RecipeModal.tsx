...
export const RecipeModal = ({ isOpen, onClose, recipe }: RecipeModalProps) => {
  console.log('RecipeModal render:', { isOpen, recipe });
  
  return (
    <DialogPrimitive.Root open={isOpen} onOpenChange={onClose}>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay 
          className="fixed inset-0 z-40 bg-black/20 animate-fade-in data-[state=closed]:animate-fade-out" 
          style={{ pointerEvents: 'none' }}
        />
        <DialogPrimitive.Content 
          className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[calc(100%-2rem)] max-w-xs outline-none animate-fade-in animate-scale-in data-[state=closed]:animate-fade-out data-[state=closed]:animate-scale-out"
